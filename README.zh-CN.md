...
| 企业级范围 | `Tenant` 和 `Workspace` CRD 当前是轻量 runtime-scope bridge，canonical 产品状态由 manager 数据库持有。 |
| Web Console | `web/` console 是当前 UX-first 产品界面，面向可视化编排、evaluation、release、provider 和治理体验。 |
...
Web Console 位于 [`web/`](./web)，是当前 Korus 的企业产品主入口，用于：

- tenant 和 workspace 导航
- 可视化 Agent 编排，支持 6 种模式和 React Flow workflow 画布
- tool/knowledge binding，包括 `topK` 和 `scoreThreshold` 等检索参数
- 模型配置，包括 `baseURL`、Secret `credentialRef.name/key`、temperature、max tokens 和 timeout
- run debugging
- evaluation 对比
- release gate
- provider 管理
...
| Phase 3 | 基于 manager 的企业产品界面，包括 Web Console、tenant、workspace、可视化编排、evaluation UX 和 provider 管理。 | **已完成。** Manager API CRUD、CRD 同步、Agent/Evaluation/Run/Provider 详情页、可视化编排工作台（6 种模式：react、router、reflection、tool_calling、plan_execute、workflow），以及交互式 React Flow workflow 画布，支持按 kind 动态配置节点。 |
...
- Helm chart 尚未打包分发；
- gateway Bearer 认证和 token-bucket 限流已实现；细粒度鉴权和请求幂等仍在演进；
- AgentRun 取消、重试、超时处理和持久 artifact storage 已实现，但 Pod 清理后的长期 trace storage 仍是 alpha 限制；
