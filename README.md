...
| Evaluation | Partial | `AgentEvaluation` now has typed dataset, baseline, evaluator, threshold gate, and reporting fields; a `Dataset` CRD can supply reusable evaluation samples with `expected` values, and the controller can create one or more managed `AgentRun` resources and compute both rule metrics and early structured metrics such as `risk_level_match` and `hazard_coverage`. Rich result reporting is still pending. |
