...
| Tenancy and workspace model | Bootstrap | `Tenant` and `Workspace` CRD skeletons now exist as the first control-plane surface for enterprise scoping, and lightweight controllers can resolve `tenantRef`, publish workspace console scope, and count tenant workspaces. | Expand the model into runtime isolation, RBAC, quotas, workspace binding, and future UI semantics. |
