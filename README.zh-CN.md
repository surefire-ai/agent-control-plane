...
| Evaluation | 部分完成 | `AgentEvaluation` 现在已有 typed dataset、baseline、evaluator、threshold gate 和 reporting 字段；controller 会解析引用，并可基于 `spec.runtime.sampleInput` 或 `spec.runtime.samples` 创建一条或多条受管 `AgentRun`，再把聚合后的运行状态回写到 status；丰富结果上报仍待实现。 |
...
| Evaluation | `AgentEvaluation` 已具备 typed dataset、baseline、evaluator、threshold gate 和 reporting 字段；controller 会解析 readiness，也能基于 `spec.runtime.sampleInput` 或 `spec.runtime.samples` 创建受管 `AgentRun`，并把 baseline revision、聚合后的 run state 和基础 gate 结果写入 status。 | 从 runtime 提供的样本集合继续扩展到真正的数据集驱动评估、丰富结果上报、revision 对比和发布门禁行为。 |
