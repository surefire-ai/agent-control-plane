...
| Evaluation | 部分完成 | `AgentEvaluation` 现在已有 typed dataset、baseline、evaluator、threshold gate 和 reporting 字段；controller 会解析引用、在提供 `spec.runtime.sampleInput` 时创建受管 `AgentRun`，并把运行状态回写到 status；多样本执行和丰富结果上报仍待实现。 |
...
| Evaluation | `AgentEvaluation` 已具备 typed dataset、baseline、evaluator、threshold gate 和 reporting 字段；controller 会解析 readiness，也能基于 `spec.runtime.sampleInput` 创建受管 `AgentRun`，并把 baseline revision、run state 和基础 gate 结果写入 status。 | 从单次 run 扩展到数据集驱动的评估执行、丰富结果上报、revision 对比和发布门禁行为。 |
