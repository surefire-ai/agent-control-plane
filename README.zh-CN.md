...
| Tenancy 与 workspace 模型 | Bootstrap | 已加入 `Tenant` 与 `Workspace` CRD skeleton，作为企业级作用域的第一层控制面接口；轻量 controller 也已能解析 `tenantRef`、发布 workspace console scope，并统计 tenant 下的 workspace 数量。 | 继续把这层模型扩展到 runtime 隔离、RBAC、quota、workspace 绑定与未来 UI 语义中。 |
