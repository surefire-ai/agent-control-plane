...
| Phase 3 | 基于 manager 的企业产品界面，包括 Web Console、tenant、workspace、可视化编排、发布流程、evaluation UX 和 provider 管理。 | 进行中。Manager API CRUD、CRD 同步、Agent 详情页、Evaluation 详情页（含关联 Runs）已完成；可视化编排工作台下一步。 |
...
- Web Console 已有 tenant/workspace CRUD、Agent 和 Evaluation 详情页（含关联 Runs），但可视化编排工作台尚未构建；
- Helm 仍是开发安装路径。
