...
| Evaluation | Partial | `AgentEvaluation` now has typed dataset, baseline, evaluator, threshold gate, and reporting fields; the controller resolves references, can create one or more managed `AgentRun` resources from `spec.runtime.sampleInput` or `spec.runtime.samples`, and folds aggregated run state back into status. Rich result reporting is still pending. |
...
| Evaluation | `AgentEvaluation` now includes typed dataset, baseline, evaluator, threshold gate, and reporting fields; the controller resolves readiness, can create managed `AgentRun` resources from `spec.runtime.sampleInput` or `spec.runtime.samples`, and writes baseline revision, aggregated run state, and basic gate results into status. | Expand from runtime-provided sample sets to dataset-backed evaluation execution, richer result reporting, revision comparison, and release-gate behavior. |
