...
| Manager backend | 已 scaffold：`cmd/manager` 和 `internal/manager` 提供最小可选 HTTP 后台，包含 health、readiness 和 info endpoints；数据模型与同步契约已记录在 `docs/architecture/` 下。 | 增加数据库连接、migration，以及最小 workspace / agent-project API。 |
